...
int test() {
  ...
  // Copy the string "t-" to pr->name
  int i;
  for (i = 0; i < sizeof(prefix) - 1 && i < sizeof(pr->name) - 1; i++) {
    pr->name[i] = prefix[i];
  }

  // Append the PID to pr->name
  if (pr->pid >= 10) {
    pr->name[i++] = '0' + (pr->pid / 10);
  }
  pr->name[i++] = '0' + (pr->pid % 10);
  pr->name[i] = '\0';

  cprintf("Il pid di questo processo è: %d \n", pr->pid);
